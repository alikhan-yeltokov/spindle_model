...
#     mu = 100.7  # Mean of the Gaussian
#     sigma = 30.3  # Standard deviation of the Gaussian
#     max_height = 1.75  # Maximum height of the distribution
...
#             # Calculate the angle between the astral MT and the spindle pole
#             angle_rad = np.arctan2(vec[1], vec[0])  # Angle in radians
#             angle_deg = np.degrees(angle_rad) % 360  # Convert to degrees and wrap to [0, 360)

#             angle_deg+=90
#             # Normalize the angle to [0, 180] using symmetry
#             if angle_deg > 180:
#                 angle_deg = 360 - angle_deg

#             # Compute the amplitude of the normal distribution at this angle
#             amplitude = max_height * np.exp(-((angle_deg - mu) ** 2) / (2 * sigma ** 2))
...
#                 # Scale the pulling force by the amplitude
#                 force *= amplitude
#                 pull_vecs[i, j] = np.multiply(vec, force)
...
# def check_spindle_1(spindle_poles, spindle_angle, cell, r, w):
#     # Debug: Print input parameters
#     # print(f"Debug: spindle_poles = {spindle_poles}")
#     # print(f"Debug: spindle_angle = {spindle_angle}")
#     # print(f"Debug: cell = {cell}")
#     # print(f"Debug: r = {r}, w = {w}")

#     # Generate spindle envelope
#     spindle = generate_spindle(spindle_poles, spindle_angle, r, w)
#     # print(f"Debug: Generated spindle envelope = {spindle}")

#     # Create polygon objects for the spindle and cell
#     spindle_polygon = Polygon(spindle)
#     cell_polygon = Polygon(cell)
#     # print(f"Debug: Spindle Polygon = {spindle_polygon}")
#     # print(f"Debug: Cell Polygon = {cell_polygon}")

#     # Calculate distances to the boundary for spindle poles
#     dist_pole1 = distance_to_boundary(spindle_poles[0], cell)
#     dist_pole2 = distance_to_boundary(spindle_poles[1], cell)
#     min_dist = min(dist_pole1, dist_pole2)
#     # print(f"Debug: Distance to boundary for pole 1 = {dist_pole1}")
#     # print(f"Debug: Distance to boundary for pole 2 = {dist_pole2}")
#     # print(f"Debug: Minimum distance to boundary = {min_dist}")
...
#         if min_dist <= config.min_cortex_dist:
#             print(f"Debug: min_dist <= config.min_cortex_dist: {min_dist <= config.min_cortex_dist}")
#         if cell_polygon.intersects(spindle_polygon):
#             print(f"Debug: cell_polygon intersects spindle_polygon: {cell_polygon.intersects(spindle_polygon)}")
#         return False  # Boundary condition violated
...
# def check_spindle_2(spindle_poles, spindle_angle, cell, r, w):
#     # Generate spindle envelope
#     spindle = generate_spindle(spindle_poles, spindle_angle, r, w)
...
#     for pole in spindle_poles:
#         if distance_to_boundary(pole, cell) <= 0.9 * config.min_cortex_dist:
...
#             if check_spindle_2(test_poles, test_angle, cell, r, w):
...
# #         ax.plot(spindle_envelope[:,0], spindle_envelope[:,1], color = 'r',label='Spindle envelope')
    ...
    if (cell_type=='endo' and c==3):
       ...
       # 'Number of astro MTs = %.1f, Number of spots= %.1f'%(2*len(astral_MTs[0]),len(spots))
        ...
        #plt.text(right_max-0.9, top-0.5, '$N_{Pull}/N_{Push}$'%(np.sum(count_pulling), np.sum(count_pushing)), fontsize = 12)  # Adjusted y-coordinate: top-0.4 → top-0.6
...
def plot_simulate(params):
    ...
    return df_angle, df_count_push, df_count_pull, df_center,df_ypos, df_ratio #, df_count_push, df_count_pull, df_length#, RMS_angle, RMS_center, cutoff, ratio_list
...
if __name__ == "__main__":
    ...
    df_angle, df_count_push, df_count_pull, df_center,df_ypos, df_ratio=plot_simulate([[a,b,r,w],motor_density,astral_number,sp_angle[int(task_id-1)],  folder_name, new_dir_path])
